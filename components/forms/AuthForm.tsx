...
interface AuthFormProps<T extends FieldValues, S extends ZodObject<any, any>> {
    ...
    onSubmit: (data: T) => Promise<ActionResponse<any>>,
...
export function AuthForm<T extends FieldValues, S extends ZodObject<any, any>>({ schema, defaultValues, formType, onSubmit }: AuthFormProps<T, S>): React.JSX.Element {
    ...
    const handleSubmit: SubmitHandler<T> = async (data) => {
        ...
        if (result?.success) {
            ...
            const errorMessage = result?.errors?.message || "An unexpected error occurred";
    ...
    return (
        <Form {...form}>
            <form onSubmit={form.handleSubmit(handleSubmit as SubmitHandler<T>)} className="mt-10 space-y-6">
